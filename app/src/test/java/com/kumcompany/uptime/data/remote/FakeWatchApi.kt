...
class FakeWatchApi: WatchApi {
    ...
    override suspend fun searchWatches(model: String): ApiResponse {
        val searchedWatches = findWatches(model)
        return ApiResponse(
            success = true,
            message = "ok",
            watches = searchedWatches
        )
    }

    private fun findWatches(model: String): List<Watch> {
        val founded = mutableListOf<Watch>()
        return if (model.isNotEmpty()) {
            watches.forEach { watch ->
                if (watch.model.lowercase().contains(model.lowercase())) {
                    founded.add(watch)
                }
            }
            founded
